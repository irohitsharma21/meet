...
from db.mongodb import get_meetings_collection, get_conversations_collection
...
async def end_meeting(
    ...
    # Allow host OR any participant in the meeting to end it
    is_host = doc["created_by"] == username or current_user["role"] == "admin"
    is_participant = any(p["username"] == username for p in doc.get("participants", []))
    ...
    # If already ended/processed, just return success (idempotent)
    if doc["status"] in (MeetingStatus.ENDED.value, MeetingStatus.PROCESSED.value):
        return {"status": doc["status"], "meeting_id": meeting_id, "duration_seconds": doc.get("duration_seconds")}
    ...
    if remaining:
        update["$push"] = {
            "transcript": {"$each": [e.model_dump() for e in remaining]}
        }

    await col.update_one({"meeting_id": meeting_id}, update)

    # Persist a conversation snapshot for easier read queries / archival
    try:
        conv_col = get_conversations_collection()
        final = await col.find_one({"meeting_id": meeting_id})
        conv_doc = {
            "meeting_id": meeting_id,
            "title": final.get("title"),
            "created_by": final.get("created_by"),
            "participants": [p.get("username") for p in final.get("participants", [])],
            "transcript": final.get("transcript", []),
            "timestamp": final.get("timestamp", datetime.utcnow()),
        }
        await conv_col.update_one({"meeting_id": meeting_id}, {"$set": conv_doc}, upsert=True)
    except Exception as e:
        print(f"⚠️ Conversation archive failed: {e}")
...
async def generate_report(
    ...
    # Allow host or any participant to generate report
    is_host = doc["created_by"] == username or current_user["role"] == "admin"
    is_participant = any(p["username"] == username for p in doc.get("participants", []))
    if not is_host and not is_participant:
        raise HTTPException(status_code=403, detail="Not authorized to generate report for this meeting")

    # Auto-end the meeting if it's still active (all participants left without ending)
    if doc["status"] == MeetingStatus.ACTIVE.value:
        try:
            remaining = await transcription_service.flush_all_buffers(meeting_id)
            transcription_service.end_session(meeting_id)
            started = doc.get("started_at")
            ended_at = datetime.utcnow()
            duration = int((ended_at - started).total_seconds()) if started else None
            col = get_meetings_collection()
            end_update: dict = {"$set": {"status": MeetingStatus.ENDED.value, "ended_at": ended_at, "duration_seconds": duration}}
            if remaining:
                end_update["$push"] = {"transcript": {"$each": [e.model_dump() for e in remaining]}}
            await col.update_one({"meeting_id": meeting_id}, end_update)
            # Re-fetch doc with updated status
            doc = await _get_meeting_or_404(meeting_id)
        except Exception as e:
            print(f"⚠️ Auto-end during report generation failed: {e}")
    ...
    try:
        ...
        print(f"❌ generate_full_report raised: {exc}")
        raise HTTPException(
            status_code=502,
    ...
    return {"status": "generated", "meeting_id": meeting_id, "analysis": analysis.model_dump()}
        ...
        while True:
            ...
            # Handle binary audio data
                ...
                if entry:
                    ...
                    await col.update_one(
                        {"meeting_id": meeting_id},
                        {"$push": {"transcript": entry.model_dump()}},
                    )

                    # Broadcast transcript to all participants (once)
                    await manager.broadcast(
                        meeting_id,
                        {"type": "transcript", "entry": entry.model_dump()},
                    )
                        ...
                        try:
                            ...
                            if action.trigger:
                                if action.next_action:
                                    await col.update_one(
                                        {"meeting_id": meeting_id},
                                        {"$push": {"ai_analysis.next_actions": action.next_action.model_dump()}}
                                    )
                                await manager.broadcast(
                                    meeting_id,
                                    {"type": "action_detected", "result": action.model_dump()},
                                )
                        except Exception as exc:
                            print(f"⚠️ Action detection error: {exc}")

                    # Retrieve recent context and schedule detection task
